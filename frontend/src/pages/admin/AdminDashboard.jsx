        ...
        <div className="admin-layout">
            <aside className={`admin-sidebar ${mobileMenu ? 'open' : ''}`}>
                <div className="admin-logo">
                    <Link to="/" className="logo"><div className="logo-icon" style={{ width: 32, height: 32, fontSize: '0.9rem' }}>💧</div><div className="logo-text" style={{ color: '#fff' }}>Aqua<span>Pure</span></div></Link>
                    <button className="mobile-menu-close" onClick={() => setMobileMenu(false)}><FiX /></button>
                </div>
                <nav className="admin-nav">
                    {navItems.map(item => (
                        <Link key={item.path} to={item.path} className={`admin-nav-item ${location.pathname === item.path ? 'active' : ''}`} onClick={() => setMobileMenu(false)}>
                            {item.icon} {item.label}
                        </Link>
                    ))}
                    <hr style={{ border: 'none', borderTop: '1px solid rgba(255,255,255,0.1)', margin: '16px 0' }} />
                    <Link to="/" className="admin-nav-item" onClick={() => setMobileMenu(false)}><FiHome /> Back to Site</Link>
                    <button className="admin-nav-item" onClick={() => { logout(); window.location.href = '/'; }}><FiLogOut /> Logout</button>
                </nav>
            </aside>
            <div className="admin-main">
                <div className="admin-topbar">
                    <button className="admin-mobile-btn" onClick={() => setMobileMenu(true)}><FiMenu /></button>
                    <h2>Dashboard</h2>
                    <div>Admin Panel</div>
                </div>
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="admin-stats-grid">
                                <div className="admin-stat-card"><div className="admin-stat-icon" style={{ background: '#dbeafe', color: '#2563eb' }}><FiDollarSign /></div><div><div className="admin-stat-value">₹{stats.totalRevenue?.toLocaleString() || 0}</div><div className="admin-stat-label">Total Revenue</div></div></div>
